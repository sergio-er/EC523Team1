...
class TrainerBase:
    ...
    def train(self):
        ...
        for ii in range(self.iters_start, self.configs.train.iterations):
            ...
            # training phase
            print(ii, data['lq'].shape, data['gt'].shape)
...
class TrainerDifIR(TrainerBase):
    ...
    def prepare_data(self, data, dtype=torch.float32, realesrgan=None, phase='train'):
        ...
        if data_type == 'paired':
            # For paired training, data already contains properly formatted lq and gt
            # Just need to ensure they're on the right device and dtype
            data = {key:value.cuda().to(dtype=dtype) for key, value in data.items()}
            return data
        
        elif realesrgan and phase == 'train':
    ...
    def backward_step(self, compute_losses, data, num_grad_accumulate, tt):
        def dif_loss_wrapper():
            return compute_losses(data['gt'], data['lq'], tt)

        with self.autocast:
            losses, z_t, z0_pred = dif_loss_wrapper()
            
            # Decode z0_pred back to image space (16x16 -> 64x64)
            with torch.no_grad():
                x0_pred = self.diffusion.decode_first_stage(z0_pred, self.first_stage_model)
            
            # Calculate LPIPS loss in image space (64x64)
            print("x0_pred.shape", x0_pred.shape, "data['gt'].shape", data['gt'].shape)
            losses["lpips"] = self.lpips_loss(
                x0_pred.clamp(-1.0, 1.0),  # Now 64x64
                data['gt'],                 # Already 64x64
            ...
            loss = sum([
                coef * loss for coef, loss in zip(self.loss_coef, losses.values())
            ])
    ...
    def training_step(self, data):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            # Now latent_resolution is already correct since we encoded the images
            latent_resolution = micro_data['gt'].shape[-1]  # Should be 16
            ...
            if last_batch or self.num_gpus <= 1:
                print("micro_data['gt'].shape", micro_data['gt'].shape, "micro_data['lq'].shape", micro_data['lq'].shape, "tt.shape", tt.shape, "noise.shape", noise.shape)
...
class TrainerDifIRLPIPS(TrainerDifIR):
    def backward_step(self, compute_losses, data, num_grad_accumulate, tt):
        ...
        with context():
            ...
            print('z0_pred.shape', z0_pred.shape)
            x0_pred = self.base_diffusion.decode_first_stage(
                    z0_pred,
                    self.autoencoder,
                    ) # f16
            self.current_x0_pred = x0_pred.detach()
            print('self.x0_pred.shape', x0_pred.shape)

            # classification loss
            print('x0_pred_clamped.shape', x0_pred.clamp(-1.0, 1.0).shape, 'data["gt"].shape', data['gt'].shape)
            losses["lpips"] = self.lpips_loss(
